...
export const createVideo = async (req, res) => {
    ...
    try {
        const [result] = await pool.query(
            'INSERT INTO recurso_video (materia_id, titulo, url, descripcion, nivel, materia) VALUES (?, ?, ?, ?, ?, ?)',
...
export const updateVideo = async (req, res) => {
    ...
    try {
        const [result] = await pool.query('UPDATE recurso_video SET materia_id = IFNULL(?, materia_id), titulo = IFNULL(?, titulo), url = IFNULL(?, url), descripcion = IFNULL(?, descripcion), nivel = IFNULL(?, nivel), materia = IFNULL(?, materia) WHERE video_id = ?', [materia_id, titulo, url, descripcion, nivel, materia, id])
        console.log(result)
