...
export const createMateria = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('INSERT INTO materia_nivel (nivel_id, nombre, imagen, fecha, autor, descripcion) VALUES (?, ?, ?, ?, ?, ?)', [nivel_id, nombre, imagen, fecha, autor, descripcion])
        res.send({
...
export const updateMateria = async (req, res) => {
    ...
    try {
        const [result] = await pool.query('UPDATE materia_nivel SET nivel_id = IFNULL(?, nivel_id), nombre = IFNULL(?, nombre), imagen = IFNULL(?, imagen), fecha = IFNULL(?, fecha), autor = IFNULL(?, autor), descripcion = IFNULL(?, descripcion) WHERE materia_id = ?', [nivel_id, nombre, imagen, fecha, autor, descripcion, id])
        console.log(result)
